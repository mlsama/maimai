...
<body id="mainframe">
	...
	<div class="w w1 header clearfix">
	    ...
	    <img src="/images/taotao-logo.gif" alt="买买商城"/></a></div>
	</div>
	<!-- /header -->
	...
	<form id="orderForm" class="hide">
		...
		<c:forEach items="${carts}" var="cart" varStatus="status">
			...
			<c:set var="buyNum"  value="${ buyNum + cart.num}"/>
		...
		<input type="hidden" name="orderShipping.receiverName" value="传智播客"/>
		<input type="hidden" name="orderShipping.receiverMobile" value="13888888888"/>
		...
		<input type="hidden" name="orderShipping.receiverAddress" value="广东省广州市天河区珠吉路58号津安创意园"/>
	</form>
	...
	<div class="step-cont">
		...
		<div class="consignee-item item-selected" consigneeId="137617472" id="consignee_index_div_137617472">
			<b></b>
			<div class="user-name">
				<div class="fl"><strong limit="4">传智播客</strong>&nbsp;&nbsp;收</div>
				<div class="fr">138****8888</div>
				<div class="clr"></div>
			</div>
			<div class="mt10" limit="15">广东广州 </div>
			<div class="adr-m" limit="30">广东省广州市天河区珠吉路58号津安创意园</div>
		...
		<div class="payment-list" id="">
			<div class="list-cont">
				...
				<li style="cursor: pointer;" onclick="save_Pay(5);">
					...
					<div class="payment-item online-payment " for="pay-method-5"
	...
	<div class="step-tit">
		<h3>送货清单</h3>
		<div class="extra-r">
			<a href="/cart/showCart.html" id="cartRetureUrl" class="return-edit ftx-05">返回修改购物车</a>
		</div>
	</div>
<div class="step-cont" id="skuPayAndShipment-cont">
   ...
   <!--满赠套装商品-->
     <!--配送方式-->
      <h4 class="vendor_name_h" id="0">商家：买买商城</h4>
      <!--单品开始-->
	 ...
	 <c:forEach items="${carts }" var="cart">     						
        <div class="goods-item goods-item-extra">
				...
				<div class="p-img">
					<a target="_blank" href="http://item.maimai.com/${cart.itemId}.html"><img src="${cart.itemImage}"/></a>
				</div>
				<div class="goods-msg">					
					<div class="p-name">
					     <a href="http://item.maimai.com/${cart.itemId}.html" target="_blank">
					           ${cart.itemTitle }				     
					     </a>
					</div>
          <div class="p-price">
         ...
         <!--配送方式-->
                  ...
                  <div class="mode-tab-nav">
                    <ul>
							...
							<span id="jdShip-span-tip" class="m-txt">买买快递<i class='qmark-icon qmark-tip' data-tips='由买买公司负责配送，速度很快，还接受上门刷卡付款服务'></i></span><b></b>
                        </li>
	...
	<div class="item">
		...
		<div class="fl">
			<select name="pickRegion" id="pickRegion_select" class="selt pickRegion_select" onchange="doEditPickReigon(this)">
						   ...
						   <option value="2:2824:0:0"  >天河区</option>
						   <option value="2:2830:0:0"  >越秀区</option>
						   <option value="2:2834:0:0"  >海珠区</option>
						   <option value="2:2823:0:0"  >荔湾区</option>
						</select>
		</div>
	</div>
	<div class="item">
...
<script type="text/javascript">
           ...
           jQuery.closeDialog();
							   ...
							   <div class="hide" id="jdItem_surpportSku">
    									...
    									<div class="goods-items">
    												...
    												<div class="p-img">
    													<a href="#none"><img src="http://img14.360buyimg.com/N4/jfs/t274/191/1323012102/354214/23ee9085/5437845cN5d6b6205.jpg"/></a>
    												</div>
    												<div class="p-name">
			         ...
			         <!--以下为买买第三方配送-->					
					 				<!--以下为第三方配送-->	 				
				<!--如果是SOP快递或者是买买中小件商品，但是不支持配送，则采用快递运输-->
				   			</div><!--dis-modes 结束-->
			<div class="clr"></div>
	   ...
	   <!-- 买买配送 -->
	   <input type="hidden" id="saveParam_jdShipmentType" name="saveParam.jdShipmentType" value="" /><!--买买配送-->
	   ...
	   <input type="hidden" id="saveParam_jdCheckType" name="saveParam.jdCheckType" value="2"/><!--如果是支票这个只是写死的[仅支持买买上门自取]-->
	   <input type="hidden" id="saveParam_jdBigItemShipTimeOffset" name="saveParam.jdBigItemShipTimeOffset" value="0"/><!--买买大家电安装时间偏移量-->
	   <input type="hidden" id="saveParam_jdBigItemInstallTimeOffest" name="saveParam.jdBigItemInstallTimeOffest" value="0"/><!--买买大家电配送时间偏移量-->	  
	   <!--311-->
	   ...
	   <input type="hidden" id="saveParam_jdBigItemNightShip" name="saveParam.jdBigItemNightShip" value="false"/><!--大家电是否支持晚间配送-->	   	   
	    <!--买买第三方配送-->
	   <input type="hidden" id="saveParam_otherShipmentType" name="saveParam.otherShipmentType" value=""/><!--买买第三方配送-->
	   <input type="hidden" id="saveParam_otherShipTime" name="saveParam.otherShipTime" value=""/><!--区分工作日，311，411-->	   
	   <input type="hidden" id="saveParam_otherBigItemShipOffset" name="saveParam.otherBigItemShipOffset" value=""/><!--买买大家电安装时间偏移量-->
	   <input type="hidden" id="saveParam_otherBigItemInstallTimeOffset" name="saveParam.otherBigItemInstallTimeOffset" value=""/><!--买买大家电配送时间偏移量-->
...
<div class="order-summary">
	<!--  预售 计算支付展现方式 begin -->
		...
		<div class="list">
			<span><em class="ftx-01">${buyNum}</em> 件商品，总商品金额：</span>
			<em class="price" id="warePriceId" v="182.60">￥${totalPrice/100 }</em>
		</div>
		<div class="list">
			<span>返现：</span>
			<em class="price" id="cachBackId" v="20.00"> -￥0.00</em>
		</div>
		<div class="list">
			<span>运费：</span>
    ...
    <div class="list" id="showFreeFreight"  style="display:none;" >
      <span>运费优惠：</span><em class="price" id="freeFreightPriceId"> -￥0.00</em>
		...
		<div class="list" id="showGiftCardPrice"  style="display:none;" >
			<span>买买卡/E卡：</span><em class="price" id='giftCardPriceId'>-￥ 0.00</em>
		</div>
		...
		<div class="list">
			<span>应付总额：</span>
			<em class="price" id="sumPayPriceId"> ￥${totalPrice/100 }</em>
		</div>
	</div>
		<div class="clr"></div>
	</div>
      ...
      <div class="sticky-wrap">
          ...
          <button type="submit" class="checkout-submit btn-1" id="order-submit" onclick="javascript:submit_Order();">
            提交订单<b></b>
                    ...
                    <span class="total">应付总额：<strong id="payPriceId">￥${totalPrice/100 }</strong>
          ...
          <div style="display:none" id="factoryShipCodShowDivBottom" class="dispatching">
            部分商品货到付款方式：先由买买配送“提货单”并收款，然后厂商发货。
          </div>
        </div>
